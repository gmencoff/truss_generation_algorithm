%function [flagPoint]=hw2_polygon_isSelfOccluded(vertex,vertexPrev,vertexNext,point)
...
if prod(all(vertexPrev==vertex))||prod(all(vertexNext==vertex))
    ...
    for iPoint=1:size(point,2)
        if vertex==point(:,iPoint)
            ...
            flagPoint(iPoint)=single(hw2_edge_angle(vertex,vertexPrev,vertexNext,'unsigned'))~=single(hw2_edge_angle(vertex,vertexPrev,point(:,iPoint),'unsigned')+hw2_edge_angle(vertex,point(:,iPoint),vertexNext,'unsigned'));
        end
    end
    
end

%Use the function hw2_edge_angle to check the angle between the segment  vertex--
