...
function [flagPoints]=polygon_isVisible(vertices,testPoints)
...
for iPoint=1:size(testPoints,2)
    for iVert=1:size(vertices,2)
        piso(iVert)=hw2_polygon_isSelfOccluded(vertices_temp(:,iVert+1),vertices_temp(:,iVert),vertices_temp(:,iVert+2),testPoints(:,iPoint));
        ...
        for indexVertex=1:size(vertices,2)
            eicindex(indexVertex)=hw2_edge_isCollision([testPoints(:,iPoint),vertices(:,iVert)],[vertices_temp(:,indexVertex+1),vertices_temp(:,indexVertex+2)],fill);
        end
